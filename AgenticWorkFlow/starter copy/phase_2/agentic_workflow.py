...
phase_1_path = os.path.join(cur_dir, '..', 'phase_1')
...
product_spec_path = os.path.join(cur_dir, 'Product-Spec-Email-Router.txt')
...
persona_product_manager_evaluation_criteria = (
...
#   4. After the loop, print a consolidated final workflow output.
print("\n*** Workflow execution completed ***")

user_stories_prompt = f"""
Using the following product specification, generate 3 to 5 user stories for the Email Router project.

Product Specification:
{product_spec}

Return user stories in this format:
As a [type of user], I want [action] so that [benefit].
"""

features_prompt = f"""
Using the following product specification, generate 3 to 5 product features for the Email Router project.

Product Specification:
{product_spec}

Return each feature in this format:
Feature Name: ...
Description: ...
"""

engineering_tasks_prompt = f"""
Using the following product specification, generate 5 to 8 engineering tasks for the Email Router project.
...
user_stories_result = routing_agent.route_prompt(user_stories_prompt)
features_result = routing_agent.route_prompt(features_prompt)
engineering_tasks_result = routing_agent.route_prompt(engineering_tasks_prompt)

final_plan = {
    "user_stories": user_stories_result,
    "features": features_result,
    "engineering_tasks": engineering_tasks_result
}
